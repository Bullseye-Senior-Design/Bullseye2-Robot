...
class HeaderHealerSwitches(Subsystem):
	def __init__(self):
		try:
			...
			self.header_switch_reader = LimitSwitchReader(pin=self.header_switch_pin, active_high=True, pull_up=True, debounce_ms=50, edge='both')
			self.healer_switch_reader = LimitSwitchReader(pin=self.healer_switch_pin, active_high=True, pull_up=True, debounce_ms=50, edge='both')
